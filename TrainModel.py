...
def test_model(model1, testdata, test_word, test_posi, test_label, index2word, resultfile ='', batch_size=10):
    index2word[0] = ''
    ...
    predictions1 = model1.predict([np.array(testdata)])

    testresult = []
    count = 0

    for si in range(0, len(predictions1)):
        sent = predictions1[si]
        # print('predictions',sent)
        ptag = []
        for word in range(len(sent)):
            next_index = np.argmax(sent[word])
    # testresult = []
...
def test_model2(model1, model2, model3, model4, model5, model6, testdata, test_word, test_posi, test_label, index2word, resultfile ='', batch_size=10):
    index2word[0] = ''
    ...
    # predictions = nn_model.predict([np.array(testdata)])
    predictions1 = model1.predict([np.array(testdata)])
    predictions2 = model2.predict([np.array(testdata)])
    predictions3 = model3.predict([np.array(testdata)])
    predictions4 = model4.predict([np.array(testdata)])
    predictions5 = model5.predict([np.array(testdata)])
    predictions6 = model6.predict([np.array(testdata)])
    testresult = []
    count = 0
    ...
    for si in range(0, len(predictions1)):
        ...
        ptag = []
        for word in range(len(sent)):
            next_index = np.argmax(1/6 * (sent[word] + predictions2[si][word] +
                                          predictions3[si][word] + predictions4[si][word] +
                                          predictions5[si][word] + predictions6[si][word]))
    # testresult = []
    # for si in range(0, len(predictions)):
    #     sent = predictions[si]
    #     # print('predictions',sent)
...
def train_e2e_model(Modelname, datafile, modelfile, resultdir, npochos=100,hidden_dim=200, batch_size=50, retrain=False):
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_acc', factor=0.1, patience=5, min_lr=0.0001)
...
def infer_e2e_model2(Modelname, datafile, mf1, mf2, mf3, mf4, mf5, mf6, resultdir, hidden_dim=200, batch_size=50):
    ...
    model1 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob),
                         targetvocabsize=len(target_vob),
                         char_W=character_W, word_W=word_W,
                         input_seq_lenth=max_s,
                         output_seq_lenth=max_s,
                         hidden_dim=hidden_dim, emd_dim=character_k)
    model1.load_weights(mf1)

    model2 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob),
                         targetvocabsize=len(target_vob),
                         char_W=character_W, word_W=word_W,
                         input_seq_lenth=max_s,
                         output_seq_lenth=max_s,
                         hidden_dim=hidden_dim, emd_dim=character_k)
    model2.load_weights(mf2)

    model3 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob),
                         targetvocabsize=len(target_vob),
                         char_W=character_W, word_W=word_W,
                         input_seq_lenth=max_s,
                         output_seq_lenth=max_s,
                         hidden_dim=hidden_dim, emd_dim=character_k)
    model3.load_weights(mf3)

    model4 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob),
                         targetvocabsize=len(target_vob),
                         char_W=character_W, word_W=word_W,
                         input_seq_lenth=max_s,
                         output_seq_lenth=max_s,
                         hidden_dim=hidden_dim, emd_dim=character_k)
    model4.load_weights(mf4)

    model5 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob),
                         targetvocabsize=len(target_vob),
                         char_W=character_W, word_W=word_W,
                         input_seq_lenth=max_s,
                         output_seq_lenth=max_s,
                         hidden_dim=hidden_dim, emd_dim=character_k)
    model5.load_weights(mf5)

    model6 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob),
                         targetvocabsize=len(target_vob),
                         char_W=character_W, word_W=word_W,
                         input_seq_lenth=max_s,
                         output_seq_lenth=max_s,
                         hidden_dim=hidden_dim, emd_dim=character_k)
    model6.load_weights(mf6)
    ...
    P, R, F, PR_count, P_count, TR_count = test_model2(model1, model2, model3, model4, model5, model6, test, test_word, test_posi, test_label, target_idex_word, resultfile,
                                                      batch_size)
    print('P= ', P, '  R= ', R, '  F= ', F)

def CCKStest(testfile, Modelname, datafile, mf1, mf2, mf3, mf4, mf5, mf6, resultdir, hidden_dim=200, batch_size=50):
    ...
    model1 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob), targetvocabsize=len(target_vob),
                                     char_W=character_W, word_W=word_W,
                                     input_seq_lenth=max_s,
                                     output_seq_lenth=max_s,
                                     hidden_dim=hidden_dim, emd_dim=character_k)
    ...
    model2 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob), targetvocabsize=len(target_vob),
                                     char_W=character_W, word_W=word_W,
                                     input_seq_lenth=max_s,
                                     output_seq_lenth=max_s,
                                     hidden_dim=hidden_dim, emd_dim=character_k)
    ...
    model3 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob), targetvocabsize=len(target_vob),
                                     char_W=character_W, word_W=word_W,
                                     input_seq_lenth=max_s,
                                     output_seq_lenth=max_s,
                                     hidden_dim=hidden_dim, emd_dim=character_k)
    ...
    model4 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob), targetvocabsize=len(target_vob),
                                     char_W=character_W, word_W=word_W,
                                     input_seq_lenth=max_s,
                                     output_seq_lenth=max_s,
                                     hidden_dim=hidden_dim, emd_dim=character_k)
    ...
    model5 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob), targetvocabsize=len(target_vob),
                                     char_W=character_W, word_W=word_W,
                                     input_seq_lenth=max_s,
                                     output_seq_lenth=max_s,
                                     hidden_dim=hidden_dim, emd_dim=character_k)
    ...
    model6 = SelectModel(Modelname, charvocabsize=len(source_char), wordvocabsize=len(source_vob), targetvocabsize=len(target_vob),
                                     char_W=character_W, word_W=word_W,
                                     input_seq_lenth=max_s,
                                     output_seq_lenth=max_s,
                                     hidden_dim=hidden_dim, emd_dim=character_k)
    ...
    for line in fr:
        ...
        if senid + 1 == txtid:
            ...
            GetResults(senid, model1, model2, model3, model4, model5, model6,
                       target_idex_word, sourc_idex_char, data_s_all, lengthlist)
            senid = txtid
            data_s_all = []
            lengthlist = []
...
def GetResults(senid, model1, model2, model3, model4, model5, model6, index2word, index2char, test, lengthlist):
    ...
    # np.array(test_word), np.array(test_posi)
    predictions1 = model1.predict([np.array(test)])
    predictions2 = model2.predict([np.array(test)])
    predictions3 = model3.predict([np.array(test)])
    predictions4 = model4.predict([np.array(test)])
    predictions5 = model5.predict([np.array(test)])
    predictions6 = model6.predict([np.array(test)])
    testresult = []
    count = 0
    ...
    for si in range(0, len(predictions1)):
        ...
        ptag = []
        for word in range(len(sent)):
            next_index = np.argmax(1/7 * (1.5*sent[word] + 1.2*predictions2[si][word] +
                                          1.2*predictions3[si][word] + 1.2*predictions4[si][word] +
                                          1.2*predictions5[si][word] + predictions6[si][word]))
            # if next_index != 0:
            next_token = index2word[next_index]
            ptag.append(next_token)
        # print('ptag--', str(ptag))
    ...
    # fw = codecs.open('./data/testdata/入院记录现病史-' + str(senid) + '.txt', 'w', encoding='utf-8')
    print(senid)
    fw.write(str(senid) + ',')
    testresult = sorted(testresult, key=lambda d: int(d.split('\t')[1]))
    for re in range(len(testresult)):
        fw.write(testresult[re])
        # print(re)
    fw.write('\n')
    fw.close()
...
if __name__ == "__main__":
    ...
    modelfile = "./data/model/model_char_word_CRF58.h5"
    ...
    if Test:
        print("test EE model....")
        print(modelfile)
        infer_e2e_model(modelname, datafile, './data/model/model_char_word_CRF21.h5', resultdir, hidden_dim=200,
                        batch_size=batch_size)

        mf1 = './data/model/model_char_word_CRF58.h5'
        mf2 = './data/model/model_char_word_CRF21.h5'
        mf3 = './data/model/model_char_word_CRF31.h5'
        mf4 = './data/model/model_char_word_CRF41.h5'
        mf5 = './data/model/model_char_word_CRF51.h5'
        mf6 = './data/model/model_char_word_CRF61.h5'
        # infer_e2e_model2(modelname, datafile, mf1, mf2, mf3, mf4, mf5, mf6, resultdir, hidden_dim=200, batch_size=batch_size)
        print('------> best_model...')
        ...
        CCKStest(testfile, modelname, datafile,
                 mf1, mf2, mf3, mf4, mf5, mf6,
                 resultdir, hidden_dim=200, batch_size=batch_size)
